...
//        btnSearchCustomer.setOnMouseClicked(e -> handleSearch());
    ...
    private void handleGetDiscount() {
        ...
        if (!isValid(listDetails)) {
            NotificationUtils.showErrorAlert("Vui lòng mua thêm tối thiểu " + String.valueOf(discountPrice.negate()) + " để dùng voucher này!", "Thông báo");
            return;
        }
        ...
        for(int i = length - 1; i >= 0; i--) {
            if (this.price.compareTo(details.get(i).getTotalPriceInvoice()) >= 0)
                if(selectedDiscount.getType() == 0) {
                    this.discountPrice = this.price.multiply((BigDecimal.valueOf(100).subtract(details.get(i).getDiscountAmount())).divide(BigDecimal.valueOf(100), RoundingMode.CEILING));
                    return;
                }
                else
                {
